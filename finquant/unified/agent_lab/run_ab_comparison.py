...
Usage:
  python finquant/unified/agent_lab/run_ab_comparison.py \\
      --cases-dir finquant/unified/agent_lab/cases/synthetic_btc_15m \\
...
def run_ab(
    *,
    ...
    governed_store_a = cycle_dir / "shared_learning_records_run_a.jsonl"
    governed_store_b = cycle_dir / "shared_learning_records_run_b.jsonl"
    ...
    run_b_paths = case_paths[split:] or case_paths   # fall back to full set if no holdout
    ...
    config_a["memory_store_path"] = str(governed_store_a)
    ...
    config_b["memory_store_path"] = str(governed_store_b)
...
def build_ab_comparison(
    *,
    ...
    return {
        ...
        "verdict_v1": _ab_verdict(a_metrics, b_metrics, snapshot_after_a, snapshot_after_b, decision_diff),
    }
...
def _ab_verdict(
    ...
    # 5. Decisions differ between runs (only if there's overlap)
    if decision_diff["overlap_cases_v1"] > 0:
        if decision_diff["decisions_changed_v1"] > 0:
            successes.append(
                f"PASS: {decision_diff['decisions_changed_v1']}/{decision_diff['overlap_cases_v1']} "
                "overlapping cases produced different decisions in Run B"
            )
        else:
            failures.append("INFO: no decision differences across overlapping cases")
...
def main() -> None:
    ...
    parser.add_argument("--run-a-fraction", type=float, default=0.7,
                        help="Fraction of cases used for Run A (rest = Run B held-out)")
