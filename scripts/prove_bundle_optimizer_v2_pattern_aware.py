#!/usr/bin/env python3
"""
Bundle Optimizer v2 — pattern-aware proof:

  1) Run pattern game (control, no bundle)
  2) Extract metrics including ``pattern_context_v1`` from replay → optimize_bundle_v2 → write bundle + proof
  3) Run pattern game with generated bundle
  4) Emit JSON: source pattern context, v2 proof fields, bundle apply, second run, comparison

  PYTHONPATH=. python3 scripts/prove_bundle_optimizer_v2_pattern_aware.py 2>/dev/null
"""
...
def main() -> int:
    ...
    args = ap.parse_args()
    begin_pml_proof_stdio("prove_bundle_optimizer_v2_pattern_aware", raw_stdout=raw_stdout_selected(args))
    ...
    tmp = Path(tempfile.mkdtemp(prefix="bundle_opt_v2_proof_"))
    ...
    pc1 = run1.get("pattern_context_v1")
    ...
    bundle_doc, proof = optimize_bundle_v2(
        metrics,
    ...
    report = {
        "directive": "prove_bundle_optimizer_v2_pattern_aware",
        "run1_pattern_context_v1": pc1,
        "pattern_context_in_metrics": metrics.get("pattern_context_v1"),
        "optimizer_v2_proof": {
            "pattern_context_used": proof_out.get("pattern_context_used"),
            "pattern_rules_triggered": proof_out.get("pattern_rules_triggered"),
            "pattern_reason_codes": proof_out.get("pattern_reason_codes"),
            "pattern_apply_diff": proof_out.get("pattern_apply_diff"),
            "v1_reason_codes": (proof_out.get("v1_proof") or {}).get("reason_codes"),
        },
        "run1_source": {
