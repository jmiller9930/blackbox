"""Layer 3 approval interface: python -m approval_interface --sandbox-db PATH --decision-token SECRET"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

_RUNTIME = Path(__file__).resolve().parents[1]
if str(_RUNTIME) not in sys.path:
    sys.path.insert(0, str(_RUNTIME))

from wsgiref.simple_server import make_server

from approval_interface.app import make_app


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Layer 3 — approval decision surface (sandbox SQLite). POST /decision requires token.",
    )
    parser.add_argument("--sandbox-db", type=Path, required=True)
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8766)
    parser.add_argument(
        "--decision-token",
        type=str,
        required=True,
        help="Required for POST /api/approvals/<id>/decision (Bearer or X-Approval-Token).",
    )
    args = parser.parse_args()
    app = make_app(args.sandbox_db, decision_token=args.decision_token)
    server = make_server(args.host, args.port, app)
    print(
        f"Layer 3 approval interface — http://{args.host}:{args.port}/\n"
        f"sandbox-db: {args.sandbox_db.resolve()}\n"
        "POST decisions require Authorization: Bearer <token> or X-Approval-Token.\n"
        "Does not execute, simulate, or mutate pipeline artifacts.",
        file=sys.stderr,
    )
    server.serve_forever()


if __name__ == "__main__":
    main()
