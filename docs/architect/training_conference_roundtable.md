...
| `scripts/trading/pyth_sse_ingest.py` | Hermes **SSE** → `market_ticks` + throttled `market_bars_5m` refresh (docker `pyth-sse-ingest`; readiness `pyth_sse_tape`; API `ANNA_PREFLIGHT_REQUIRE_PYTH_SSE`) |
