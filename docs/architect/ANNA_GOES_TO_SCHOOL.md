...
|--------|------|--------------|--------|
| **Baseline (Jupiter_2 Sean policy)** | **Sean’s rules = Jupiter trade policy.** Engine: ``jupiter_2_sean_policy.evaluate_jupiter_2_sean`` via ``sean_jupiter_baseline_signal`` (adapter). Env ``BASELINE_LEDGER_SIGNAL_MODE=sean_jupiter_v1`` is a **historic mode name** only (not “policy v1”). **Catalog id** ``jupiter_2_sean_perps_v1``; Supertrend **(10,3)**, EMA200, RSI 14, simple TR ATR ratio ≥ 1.35. Legacy catalog label ``jupiter_supertrend_ema_rsi_atr_v1`` may appear on old rows. Legacy mechanical long: ``BASELINE_LEDGER_SIGNAL_MODE=legacy_mechanical_long``. **Jupiter only** — do not conflate with deprecated venue paths (see §1.1.2.1). **Canonical rule text:** [`docs/trading/jupiter_2_baseline_operator_rules.md`](../trading/jupiter_2_baseline_operator_rules.md). | **No** live venue (paper measurement) | Separate from Anna **sideline** strategies. |
