...
| **4.6.3.2** | **Learning Core Extraction** (`agent_learning_core`) | **Active (under architect review gate)** — Part A containment slice built in working tree; canonical adoption pending pre-verification + architect commit authorization (see Phase 4.6.3.2 below) |
...
**Phase 4.6.3.2 — Learning Core Extraction (`agent_learning_core`) — Directive 4.6.3.2 (active, Part A under gate):** This phase is now active for the first enforceable containment slice: lifecycle baseline + enforcement for reusable knowledge (`candidate -> under_test -> validated|rejected`, validated-only reuse). **Gate rule:** current Part A code is treated as *built / under review / pending architect gate* until pre-verification and commit authorization are complete. **Boundaries remain strict:** no trading execution changes, no Slack/Telegram routing/persona changes, no Anna response contract changes, no autonomous learning influence. **Expansion timing for other agents:** roadmap/documentation stubs only until Part A acceptance; post-acceptance order is DATA first, Cody second, Mia and Billy remain stubbed unless explicitly authorized. **One-line definition:** *`agent_learning_core` = shared context + memory + validation system for all agents.* **One-line rule:** *Build the smallest enforcement layer that prevents incorrect reuse before enabling broader learning.*
