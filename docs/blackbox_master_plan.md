...
| **4.6.3.3** | Messaging interface abstraction | **Infrastructure (directive)** — [`directives/directive_4_6_3_3_messaging_interface.md`](architect/directives/directive_4_6_3_3_messaging_interface.md); CLI + normalized output; Telegram as **adapter**; **depends on** 4.6.3.1 code closure; **Slack / OpenClaw out of scope** for this directive |
...
- **Next infrastructure focus:** Phase **4.6.3.3** — messaging abstraction (CLI + adapters); see [`directives/directive_4_6_3_3_messaging_interface.md`](architect/directives/directive_4_6_3_3_messaging_interface.md).
...
**Phase 4.6.3.3 — Messaging interface abstraction (infrastructure leaf):** Decouple Anna from Telegram; introduce **`messaging_interface/`** (`base_interface`, `cli_adapter`, `telegram_adapter`). Anna is **not** aware of transport. **Normalized output** (non-negotiable): compare `interpretation.summary`, `answer_source`, intent, topic, `limitation_flag` (if present); adapters **format only**. **CLI** is the **primary validation surface**; phase **cannot close** without CLI validation (`echo "…" | python -m messaging_interface.cli_adapter`). **Slack** and **OpenClaw gateway** integration are **out of scope** for this directive. **Depends on** 4.6.3.1 (code closure). Canonical spec: [`docs/architect/directives/directive_4_6_3_3_messaging_interface.md`](architect/directives/directive_4_6_3_3_messaging_interface.md).

**Phase 4.6.3.2 — Learning Core Extraction (`agent_learning_core`) — Directive 4.6.3.2 (stub only):** **Do not implement code yet.** This records a **future shared module** — not an agent and **not** an LLM — that standardizes how **all** agents handle **context**, **memory** (store / reuse), **LLM discipline** (rules → then model), **human feedback**, and **validation lifecycle** (e.g. candidate → approved → rejected). Long-term it may serve **Anna**, **DATA**, **Cody**, and future agents; be **embedded**, then a **shared library**, then optionally an **external API service**; and be **extractable** to other projects. **Boundaries:** it does **not** replace the LLM, generate raw intelligence, or override deterministic rules; it **manages how intelligence is applied** and **how learning is stored and validated**. **Current status:** concept on master plan only; **Anna remains the proving ground** for today’s `anna_modules` / pipeline behavior. **Implementation trigger (not blocking current work):** build **after** **4.6.3.X** Telegram validation is complete, Anna is **stable enough** for the team, and **learning-loop behavior** (4.4 visibility + Anna memory paths) is **verified** to expectations. **Future acceptance (when built):** reusable by **≥2 agents**, universal context rules, hallucination resistance via context + validation, memory lifecycle as above, decoupled from Anna-only logic. **One-line definition:** *`agent_learning_core` = shared context + memory + validation system for all agents.* **One-line rule:** *Build the intelligence once. Reuse it everywhere.*
