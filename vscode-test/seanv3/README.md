...
- **Entry policy (runtime):** **`analog_meta.jupiter_active_policy`** is the **only** execution source (deployment ids under Jupiter in **`kitchen_policy_deployment_manifest_v1.json`**). Missing row or empty value = **standby** (no default / no `SEAN_JUPITER_POLICY` fallback for execution). Clear explicitly with `POST {"policy":""}`. Legacy slot **`jup_mc_test`** was removed from the shared registry. **Operator API:** `GET /api/v1/jupiter/policy` (includes **`runtime_execution_state`**, **`loader_ok`** / **`loader_error`**), `POST /api/v1/jupiter/active-policy` (Bearer `JUPITER_OPERATOR_TOKEN`). Dashboard: **Set active Jupiter policy**.
