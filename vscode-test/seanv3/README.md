...
**API contract (Jupiter):** observability (**`GET`**) + **one controlled write**: **set active deployment id** (policy artifact binding). Reads are open within your network policy; **`JUPITER_WEB_READ_ONLY=1`** (default on **`jupiter-web`**) blocks **`POST /api/operator/*`** (wallet, funding, stake). The **sole write** is **`POST /api/v1/jupiter/active-policy`** (alias **`POST /api/v1/jupiter/set-policy`**) with **`Authorization: Bearer`** and body **`{"policy":"<deployed_runtime_policy_id>"}`** where the id appears under Jupiter in **`renaissance_v4/config/kitchen_policy_deployment_manifest_v1.json`**. The engine (**`BBT_v1`** display id, or override **`SEAN_ENGINE_DISPLAY_ID`**) is separate from that id: it loads **`evaluator.mjs`** from the bound submission. The write records **`analog_meta.jupiter_active_policy`** only — it does **not** mutate trade history or rewrite bars. **`GET /api/v1/jupiter/policy`** returns **`contract`**, **`allowed_policies`**, **`engine_display_id`**, **`engine_online`**, and the active deployment. Compose loads **`lab_operator_token.env`**. Set **`JUPITER_WEB_READ_ONLY=0`** to re-enable dashboard wallet POSTs. Bearer is **not** a Solana wallet key.
...
- **Entry policy (runtime):** **`analog_meta.jupiter_active_policy`** is the primary source of truth (deployment ids listed under Jupiter in **`kitchen_policy_deployment_manifest_v1.json`**, e.g. `jup_v4` | `jup_v3` | `jup_mc2` | `jup_pipeline_proof_v1` | `jup_kitchen_mechanical_v1`). Legacy slot **`jup_mc_test`** was removed from the shared registry; do not use it for new assignments. If unset, **`SEAN_JUPITER_POLICY`** is used (`jupiter_4` / `jupiter_3` aliases), then default **`jup_v4`**. The engine reads this **each processing cycle** (no long-lived cache). **Operator API:** `GET /api/v1/jupiter/policy` (includes **`loader_ok`** / **`loader_error`** — proves evaluator import), `POST /api/v1/jupiter/active-policy` (alias `set-policy`) (Bearer `JUPITER_OPERATOR_TOKEN`). Dashboard: **Set active Jupiter policy**.
