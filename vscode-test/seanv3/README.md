...
**Jupiter** (read-only web app; container **`jupiter-web`):** binds **HTTP** on **707** on the **lab host** (no TLS in the Node app). **`/`** = front door (Jupiter image + link). **`/dashboard`** = full operator UI (TUI parity). **Canonical WAN URL (Internet):** **`http://jupv3.greyllc.net:737/…`** (**WAN :737 → host :707**). **VPN/LAN:** **`http://clawbot.a51.corp:707/…`**. **Proof:** `curl -sS http://jupv3.greyllc.net:737/health` (or clawbot :707 from VPN) — same URL class as the browser, **not** loopback on your laptop. Default **`JUPITER_WEB_PORT=707`**. Committed **`JUPITER_PUBLIC_BASE_URL`** targets the WAN hostname. **`/api/summary.json`**, wallet, position, trades. Deploy on **clawbot:** `docker compose up -d` in this directory. Editor-only dev clone: **`npm run jupiter`** (may need **`sudo`** for **707** on Linux).
...
- **`session`** — Sign in at **`/auth/login`** with **username** (**`JUPITER_AUTH_USERNAME`**, default **`admin`**) and password (hashed in SQLite). **`JUPITER_AUTH_EMAIL`** is **only** for **forgot-password** emails (default **`jmiller9930@om3.us`**), not for the login field. **Forgot password** at **`/auth/forgot`** emails a reset link via **[Resend](https://resend.com)** when **`RESEND_API_KEY`** is set; otherwise the reset URL is **logged to `docker logs jupiter-web`**. Lab defaults include **`JUPITER_AUTH_BOOTSTRAP_PASSWORD`** to seed the first login; remove after first deploy or rotate via reset. **Required:** **`JUPITER_SESSION_SECRET`** (≥16 chars) in **`vscode-test/seanv3/.env`**. **`GET /health`** and **`/static/jupiter_front_door.png`** stay unauthenticated.
- **`basic`** — Legacy **HTTP Basic Auth** if **`JUPITER_WEB_LOGIN_USER`** and **`JUPITER_WEB_LOGIN_PASSWORD`** are set (no committed defaults).
- **`none`** — No browser gate (network/VPN only).

Operator **`JUPITER_OPERATOR_TOKEN`** (Bearer) for **`POST /api/v1/jupiter/active-policy`** is **separate** from browser login.
...
2. **Prove reachability the same way everywhere:** `curl -sS http://jupv3.greyllc.net:737/health` from the Internet (or `http://clawbot.a51.corp:707/health` on VPN). Or over SSH: `ssh jmiller@clawbot.a51.corp 'curl -sS http://clawbot.a51.corp:707/health'`. If that fails, on **clawbot** run **`docker compose ps`** and **`docker logs jupiter-web --tail 80`** in **`~/blackbox/vscode-test/seanv3`**, then **`docker compose up -d --build`** after **`git pull`** succeeds.
3. **`jupsync.py`** uses the default health URL (`JUPSYNC_JUPITER_HEALTH_URL`, default **`http://jupv3.greyllc.net:737/health`**) on the remote host after deploy — not `127.0.0.1`. If the lab host cannot curl its own WAN URL (hairpin NAT), set **`JUPSYNC_JUPITER_HEALTH_URL=http://127.0.0.1:707/health`** for checks only, or **`http://clawbot.a51.corp:707/health`** on VPN.
...
**Consistent update process:** commit in your clone → **`python3 scripts/jupsync.py`** from repo root. That **pushes** to `origin`, **SSHs to clawbot**, **`git pull`** in `~/blackbox`, then **`docker compose up -d --build`** in **`vscode-test/seanv3`**. The script then **curls the lab health URL** (default **`http://jupv3.greyllc.net:737/health`** — override with **`JUPSYNC_JUPITER_HEALTH_URL`**). **Verify in a browser:** **`http://jupv3.greyllc.net:737/`** (WAN) or **`http://clawbot.a51.corp:707/`** (VPN). Skip the health step with **`--skip-health`**.
...
- **Entry policy (runtime):** **`analog_meta.jupiter_active_policy`** is the primary source of truth (`jup_v4` | `jup_v3` | `jup_mc_test` | `jup_mc2`). If unset, **`SEAN_JUPITER_POLICY`** is used (`jupiter_4` / `jupiter_3` / `jupiter_mc_test` aliases), then default **`jup_v4`**. The engine reads this **each processing cycle** (no long-lived cache). **Operator API:** `GET /api/v1/jupiter/policy`, `POST /api/v1/jupiter/active-policy` (alias `set-policy`) (Bearer `JUPITER_OPERATOR_TOKEN`). Dashboard: **Set active Jupiter policy**.
