...
**Operator writes (Jupiter):** compose loads **`lab_operator_token.env`** (committed lab secret + optional Bearer prefill). Set **`JUPITER_OPERATOR_TOKEN`** on **`jupiter-web`** if you override elsewhere. **`JUPITER_WEB_READ_ONLY=1`** (default on **`jupiter-web`** in compose) blocks **`POST /api/operator/*`** (wallet, funding mode, paper stake); **`GET`** APIs stay open. **Strategy policy** can still be changed by other applications: **`POST /api/v1/jupiter/set-policy`** with header **`Authorization: Bearer <same token>`** and JSON body **`{"policy":"jup_v4"}`** (or **`jup_v3`**, **`jup_mc_test`**). **`GET /api/v1/jupiter/policy`** reads the active policy. Set **`JUPITER_WEB_READ_ONLY=0`** to re-enable dashboard wallet/funding POSTs. Bearer is **not** a Solana wallet key.
