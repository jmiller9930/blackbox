...
**Operator writes (Jupiter):** compose loads **`lab_operator_token.env`** (committed lab secret + optional Bearer prefill). Set **`JUPITER_OPERATOR_TOKEN`** on **`jupiter-web`** if you override elsewhere, then use the **Operator token** field in the browser (Bearer — **not** a Solana wallet key). The **Wallet & funding** panel (below Operator token) has **Paper wallet** vs **Chain wallet** toggles (`POST /api/operator/funding-mode`), **Register pubkey** (`POST /api/operator/paper-wallet`), and **paper stake** — no **`KEYPAIR_PATH`** required for the pubkey gate. Policy switch: `GET`/`POST /api/v1/jupiter/policy`.
