...
- **`session`** — Sign in at **`/auth/login`** (email = **`JUPITER_AUTH_EMAIL`**, default **`jmiller9930@om3.us`**). Password is stored hashed in SQLite (`jupiter_web_auth_*` tables). **Forgot password** at **`/auth/forgot`** sends a reset link to that email via **[Resend](https://resend.com)** when **`RESEND_API_KEY`** is set; otherwise the reset URL is **logged to `docker logs jupiter-web`** (one-hour token). Set **`JUPITER_PUBLIC_BASE_URL`** (no trailing slash) so links work from email (e.g. WAN URL). **Required:** **`JUPITER_SESSION_SECRET`** (≥16 chars) in **`vscode-test/seanv3/.env`** (gitignored) or exported before **`docker compose`**. One-time **`JUPITER_AUTH_BOOTSTRAP_PASSWORD`** can seed the first password; remove after use or use email reset. **`GET /health`** and **`/static/jupiter_front_door.png`** stay unauthenticated for probes/assets.
