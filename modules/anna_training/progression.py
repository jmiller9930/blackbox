...
def suggest_next_focus(
    *,
    ...
    if not g12.get("pass"):
        ...
        hints.append("Increase decisive paper trades or improve win rate until grade-12 gate PASS.")
        if g12.get("blockers"):
            hints.extend(str(b) for b in (g12.get("blockers") or [])[:5])

    cid = (curriculum_id or "").strip()
    if g12.get("pass") and cid == "grade_12_paper_only":
        focus = "eligible_to_advance_curriculum"
        hints.append(
            "Grade-12 numeric gate PASS — you may advance to bachelor_paper_track_v1 when human policy allows "
...
def bachelor_eligibility_report(
    *,
    ...
    """Whether bachelor assignment is allowed (prereq + gate)."""
    g12 = evaluate_grade12_gates()
