...
def anna_narrate_pattern_report(report_markdown: str, *, timeout: float = 180.0) -> tuple[str | None, str | None]:
    ...
    **factual** explanations. Referee numbers must still come only from the facts block.
    ...
    prompt = (
        ...
        + "**AUTHORITATIVE RULES**\n"
        + "- **ANNA PERCEPTION** (if present above): you only get a **short visible window** of OHLCV (latest bars), not the full historical tape. Do not claim you saw every bar of the replay.\n"
        ...
        + "- **RETROSPECTIVE / BATCH SCORECARD** (if present): **memory** of prior operator notes and batch runs — use to avoid repeating the same experiment; not a full chart.\n"
        + "- **REFEREE FACTS** (below): the **only** source for numeric outcomes (wins, losses, PnL, trades, checksums). These summarize a **full** replay; they are not a bar-by-bar feed. Never invent or alter those numbers.\n"
...
def anna_answer_operator_question(question: str, *, timeout: float = 180.0) -> tuple[str | None, str | None]:
    ...
    prompt = (
        ...
        + "**RULES**\n"
        + "- If **ANNA PERCEPTION** is present, you only have that **short OHLCV window** plus docs/memory sections — not the entire stored tape.\n"
