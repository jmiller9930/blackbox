...
| **Anna + LLM (factual help)** | `player_agent` + Ollama: **`anna_narrate_pattern_report`** and **`anna_answer_operator_question`**. Repo prefix from `scripts/agent_context_bundle.py` — **`pattern_game`** profile now includes game spec + `QUANT_RESEARCH_AGENT_DESIGN.md` + `context_memory.py`. If `ANNA_CONTEXT_PROFILE` is empty/none, calls default it to **pattern_game** so she is not “alone” without docs. CLI: ``--ask "…"`` for a standalone question. She does **not** change Referee scores. |
