...
| **Hypothesis gate** | `PATTERN_GAME_REQUIRE_HYPOTHESIS=1` or `validate_scenarios(..., require_hypothesis=True)` — blocks scenarios with empty `agent_explanation.hypothesis`. **Local web UI defaults to requiring** a non-empty hypothesis on parallel runs; set **`PATTERN_GAME_REQUIRE_HYPOTHESIS=0`** to disable. Shipped examples include a starter `hypothesis` string. |
