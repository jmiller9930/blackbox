...
from renaissance_v4.game_theory.parallel_runner import (
    PATTERN_LEARNING_RECIPE_ID,
    REFERENCE_COMPARISON_RECIPE_ID,
    validate_reference_comparison_batch_results,
)


def test_validate_learning_harness_skipped_for_custom_recipe() -> None:
    validate_reference_comparison_batch_results(
...
def test_validate_reference_comparison_raises_when_zero_candidates() -> None:
    ...
    with pytest.raises(RuntimeError, match="learning_harness_invalid"):
        validate_reference_comparison_batch_results(
            [row], operator_recipe_id=REFERENCE_COMPARISON_RECIPE_ID
        )


def test_validate_pattern_learning_raises_when_zero_candidates() -> None:
    row = {
        "ok": True,
        "scenario_id": "s1",
        "learning_run_audit_v1": {
    ...
    with pytest.raises(RuntimeError, match="learning_harness_invalid"):
        validate_reference_comparison_batch_results(
            [row], operator_recipe_id=PATTERN_LEARNING_RECIPE_ID
        )
