...
| **GT_DIRECTIVE_016** | **L1 road data:** **`GET /api/student-panel/l1-road`** — fingerprint × **brain profile** × **`llm_model`** aggregates; **A \| B** vs same-fingerprint baseline; API **`legend`**; single-pass scorecard read (`student_panel_l1_road_v1`). **Deferred / next:** embedded L1 row bands + optional scorecard denorm (Anna align, mem/ctx flags) per original §16.2. | **Active — v1 road API shipped; OPEN** | `directives/GT_DIRECTIVE_016_l1_road_data_system_student_split_v1.md`; proof `docs/proof/exam_v1/GT_DIRECTIVE_016_operator_proof_l1_road_v1.md`. |
