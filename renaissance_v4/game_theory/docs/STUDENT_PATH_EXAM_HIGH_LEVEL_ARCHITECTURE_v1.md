...
**Status:** v1.30 — **GT_DIRECTIVE_024** series (**OPEN**): micro-directives **024A–024E** — Student execution authority design, `student_execution_intent_v1`, Student-controlled replay lane, scorecard/L1 lane separation, LangGraph trace proof. Canonical spec: **`renaissance_v4/game_theory/directives/GT_DIRECTIVE_024_series_student_execution_authority_v1.md`**. **GT_DIRECTIVE_023** learning effectiveness audit (**CLOSED**): `GET /api/training/learning-effectiveness`, materialize, `learning_effectiveness_report_v1`. **GT_DIRECTIVE_022** training export (**CLOSED**). **GT_DIRECTIVE_021** spec locked (not implemented). **§1.0.2** LLM mandatory thesis on sealed object. **016** CLOSED; **016R1** / **019** (reserved). **§18.1a** dictionary. **015** CLOSED; **018** v1. **017** precondition shipped.
...
| Confidence (band) | **`confidence_band`**: `low` \| `medium` \| `high` | **Required** when `student_brain_profile_v1` = **`memory_context_llm_student`** (Ollama path); optional for stub / non-LLM profiles. |
| Why / conflict (structured) | **`supporting_indicators`**, **`conflicting_indicators`**: `list[str]` | **Required** (may be `[]`) for **LLM profile**; optional otherwise. |
| Context fit | **`context_fit`**: short string | **Required** for **LLM profile**. |
| Invalidation | **`invalidation_text`** | **Required** for **LLM profile**; distinct from **`reasoning_text`**. |
| Action (ENTER / NO_TRADE) | **`act`** bool + **`direction`**; **`student_action_v1`**: `enter_long` \| `enter_short` \| `no_trade` | **Required** for **LLM profile**; must agree with `act` / `direction`. Stub sets it only when unambiguous (optional fields omitted on non-LLM paths). |
...
#### 1.0.3 GT_DIRECTIVE_024 series — execution truth vs scored execution (micro-directives)

**Code truth today (parallel Run Exam path):** Referee **control replay** owns the trades that are aggregated into batch scorecard metrics (`referee_win_pct`, `expectancy_per_trade`, etc.); the Student seam produces `student_output_v1` **after** that replay. Identical scores across **baseline**, **memory_context_student**, and **memory_context_llm_student** for the same fingerprint are therefore **consistent** with shadow-only **scored** execution until 024C+ lands.

**Product direction:** Move the Student from **observer** to **execution authority** only through staged micro-directives **024A → 024E** (design contract → intent schema → replay lane → scorecard/L1 separation → LangGraph trace proof). Required vocabulary and acceptance bar: **`GT_DIRECTIVE_024_series_student_execution_authority_v1.md`**. Do not implement a single vague “wire Student into `run_manifest_replay`” patch; 024A must record the **code-grounded** injection choice (apply/candidate vs new row type vs narrow hook) before replay edits.

#### 1.0.2 LLM profile enforcement (precondition for **GT_DIRECTIVE_017**)

When **`student_brain_profile_v1` = `memory_context_llm_student`**, `emit_student_output_via_ollama_v1` **rejects** (returns errors; **no** stub fallback) unless **all** thesis keys in **`THESIS_REQUIRED_FOR_LLM_PROFILE_V1`** (`contracts_v1.py`) are present and pass shape validation. Seam audit exposes **`llm_student_output_rejections_v1`**. Persisted **`student_learning_record_v1.student_output`** carries the full thesis object. **L3** (`student_panel_d14.build_student_decision_record_v1`) exposes flat **`student_*`** aliases and adds **`data_gaps`** when an LLM-profile run lacks a complete stored thesis.

**Non-LLM profiles:** thesis keys remain **optional**; deterministic stub output is unchanged aside from optional `student_action_v1` when unambiguous.
...
**Canonical doc:** `renaissance_v4/game_theory/docs/STUDENT_PANEL_DICTIONARY_v1.md` (v1.3+) — tables for **L1 / L2 / L3**, scorecard column shorthand (**Sys BL %**, **Run TW %**, **>BL**, harness vs Student handoff, **Groundhog state**), **brain profiles** and **`exam_run_contract_v1`**, **Decision A — directional thesis** + **`student_output_v1`** thesis keys (**LLM profile mandatory** — §1.0.2), **L1 road** (`GET /api/student-panel/l1-road`: bands, `pass_rate_percent`, E/P proxies, `legend`), L2 **direction align**, memory/context/LLM roles, and **`data_gap`**. **UI:** Pattern Machine page → **Student → learning → outcome** fold → link **Student panel dictionary** (also under Level 1 legend: **Dictionary**). **Same content in-browser:** `GET /docs/student-panel-dictionary` on the Flask host (e.g. `http://127.0.0.1:8765/docs/student-panel-dictionary`).

### 18.1b L1 road aggregation (**GT_DIRECTIVE_016**)

**Routes:** `GET /api/student-panel/l1-road` — full `student_panel_l1_road_v1` (**`groups`**, **`road_by_job_id_v1`**, **`legend`**, **`data_gaps`**). **`GET /api/student-panel/runs`** — embeds **`l1_road_v1`** overlay (`schema` = `student_panel_l1_road_runs_overlay_v1`: **`legend`**, **`road_by_job_id_v1`**, top-level gaps) so the **Level 1** embedded table can join per-`job_id` **without** a second client fetch. Implementation: **`student_panel_l1_road_v1`** — scorecard-only aggregation (no per-row learning-store scan). **E** = mean `expectancy_per_trade`; **P** = optional `student_l1_process_score_v1`; **A \| B** bands vs same-fingerprint baseline anchor. **UI:** L1 columns **Profile**, **LLM**, **Road**, **Anchor**, **Road gaps** + legend block from **`l1_road_v1.legend`** (not hardcoded copy). Proof: `docs/proof/exam_v1/GT_DIRECTIVE_016_operator_proof_l1_road_v1.md`. **016 v1 CLOSED** — scorecard denorm, exam-pack E/P vs proxies, formal §16.1 dual-band chrome are **deferred in writing** to **`GT_DIRECTIVE_016R1`** / **`GT_DIRECTIVE_019`** (reserved identifiers; see `directives/GT_DIRECTIVE_016_l1_road_data_system_student_split_v1.md` **Deferred work register**).
...
| **GT_DIRECTIVE_015** | **Run exam + Student brain profile contract:** three profiles; nested **`student_llm_v1`**; legacy **inputs** normalize; memory → context → optional LLM → sealed output → Referee; **metadata-only** skip-cold (honest label; not physical Referee skip). **Deferred (do not block 016):** E/P comparison surface (product / likely **016** or dedicated directive); physical cold Referee skip; refine-then-seal → **GT_DIRECTIVE_021**; expanded **`llm_role`** beyond single-shot; learning-loop governance stays **018**. | **Accepted v1 / CLOSED** | `directives/GT_DIRECTIVE_015_run_exam_baseline_skip_contract_v1.md`. |
| **GT_DIRECTIVE_016** | **L1 road data:** **`GET /api/student-panel/l1-road`** + **`l1_road_v1`** on **`GET /api/student-panel/runs`**; **`road_by_job_id_v1`**; L1 UI columns (Profile, LLM, Road, Anchor, Road gaps) + **legend from API**. **Deferred:** scorecard denorm (§16.2) + §16.1 dual-band → **`GT_DIRECTIVE_016R1`** (reserved); exam-pack E/P on scorecard + L1 → **`GT_DIRECTIVE_019`** (reserved). | **Accepted v1 / CLOSED** | `directives/GT_DIRECTIVE_016_l1_road_data_system_student_split_v1.md`; proof `docs/proof/exam_v1/GT_DIRECTIVE_016_operator_proof_l1_road_v1.md`. |
| **GT_DIRECTIVE_017** | **L3 `data_gap` closure:** gap register matrix (code → producer → acceptance); shrink happy-path `data_gaps[]`; wire exports instead of guessing; couples to L1 strong claims per directive text. **PRECONDITION shipped:** §1.0.2 LLM directional thesis enforce + L3 surface (`student_panel_d14`). | **Planned — start after precondition verified in prod** | `directives/GT_DIRECTIVE_017_student_l3_datagap_closure_matrix_v1.md` (**PRECONDITION** section). |
...
| **GT_DIRECTIVE_021** | **Refine-then-seal (LLM):** in-memory **draft** = same `student_output_v1` shape, **base** `validate_student_output_v1` only (no thesis on draft); **one** **refine** HTTP pass with **identical** gen params as draft (`temperature = 0.15` unchanged); **full** validation including thesis on **refined** object only; **reject trade** if refine fails; **no** L3 draft, **no** learning on draft; audit **`llm_refine_attempts_v1`**; cap semantics explicit in tests. | **Spec locked — build pending** | `directives/GT_DIRECTIVE_021_reasoning_refine_then_seal_v1.md`. |
| **GT_DIRECTIVE_022** | **Training export:** `training_record_v1` from **promote-only** learning rows + **done** scorecard + LLM thesis complete; deterministic NDJSON; `GET /api/training/export` (+ `download=1`), `POST …/materialize`; paths `PATTERN_GAME_TRAINING_DATASET_V1` / default runtime file. **No** training / LLM behavior change. | **Accepted v1 / CLOSED** | `directives/GT_DIRECTIVE_022_llm_training_readiness_pipeline_v1.md`; proof `docs/proof/exam_v1/GT_DIRECTIVE_022_training_export_v1.md`. |
| **GT_DIRECTIVE_023** | **Learning effectiveness proof:** read-only `learning_effectiveness_report_v1` from scorecard + learning store (optional training JSONL **line count** only); time slopes, profile deltas, promotion next-run split, stability variance, heuristic `verdict_v1`; `GET /api/training/learning-effectiveness` (`summary=1`), `POST …/materialize`; `PATTERN_GAME_LEARNING_EFFECTIVENESS_REPORT_V1`. **No** runtime behavior change. | **Accepted v1 / CLOSED** | `directives/GT_DIRECTIVE_023_learning_effectiveness_proof_v1.md`; proof `docs/proof/exam_v1/GT_DIRECTIVE_023_learning_effectiveness_v1.md`. |
| **GT_DIRECTIVE_024** | **Student execution authority (micro-series 024A–024E):** design contract → `student_execution_intent_v1` → Student-controlled replay **lane** (baseline/control unchanged) → scorecard/L1/E–P **lane separation** → LangGraph trace verdicts (`STUDENT AUTHORITY PROVEN` / `STUDENT SHADOW ONLY` / …). **No** vague single-hook replay patch; order **A→B→C→D→E**. | **OPEN** | `directives/GT_DIRECTIVE_024_series_student_execution_authority_v1.md`; architecture cross-ref **§1.0.3**. |
...
| v1.26 | **§1.0.2** — LLM profile mandatory thesis (reject / `llm_student_output_rejections_v1`; no stub fallback); L3 flat fields + `data_gaps`; tests/fixtures/proof; **017** directive **PRECONDITION**; D14 field-sources; dictionary v1.3. |
| v1.27 | **GT_DIRECTIVE_021** — refine-then-seal **binding spec** (draft = same shape, base validate only; refine replaces draft; one sealed object; no L3 draft; 018 unchanged on final; failure = reject trade; `llm_refine_attempts_v1`; temp 0.15 unchanged); §18.4 row; **015** deferred pointer to **021**. |
| v1.28 | **GT_DIRECTIVE_022** — training readiness export (`training_export_v1`, `/api/training/export`, materialize, tests + proof); promote-only + done + thesis rules; §18.4 row. |
| v1.29 | **GT_DIRECTIVE_023** — learning effectiveness report (`learning_effectiveness_report_v1`, `/api/training/learning-effectiveness`, materialize, tests + proof); read-only audit gate before training; §18.4 row. |
| v1.30 | **GT_DIRECTIVE_024** series (**OPEN**) — micro-directives **024A–024E** (`GT_DIRECTIVE_024_series_student_execution_authority_v1.md`): execution authority, `student_execution_intent_v1`, Student lane replay, scorecard/L1 separation, trace proof; **§1.0.3** code truth vs staged implementation. |
