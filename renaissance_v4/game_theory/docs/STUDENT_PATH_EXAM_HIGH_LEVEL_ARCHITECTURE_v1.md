...
**Status:** v1.22 — **GT_DIRECTIVE_016** partial: L1 table + **`l1_road_v1`** on **`/api/student-panel/runs`**. **§18.1a** dictionary. **015** CLOSED; **018** v1. **§1.2** good learning loop. **009a** shipped.
...
| **GT_DIRECTIVE_016** | **L1 road data:** **`GET /api/student-panel/l1-road`** + **`l1_road_v1`** on **`GET /api/student-panel/runs`**; **`road_by_job_id_v1`**; L1 UI columns (Profile, LLM, Road, Anchor, Road gaps) + **legend from API**. **OPEN:** scorecard denorm (§16.2), exam-pack E/P vs proxies, formal A\|B band layout or **written deferral**; **017** gated. | **Active — PARTIAL acceptance; OPEN** | `directives/GT_DIRECTIVE_016_l1_road_data_system_student_split_v1.md`; proof `docs/proof/exam_v1/GT_DIRECTIVE_016_operator_proof_l1_road_v1.md`. |
