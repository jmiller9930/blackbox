...
Usage:
  ...
  print(decision.action)       # ENTER_LONG | NO_TRADE | HOLD | EXIT
  print(decision.confidence)   # 0.0–1.0
  print(decision.thesis)       # reasoning narrative from winning ToT branch
  print(decision.source)       # rule | llm_tot | hybrid | guard_vetoed
  print(decision.regime)       # trending_up | trending_down | ranging | volatile

Self-test (runs without LLM, no network required):
  python3 reasoning_module_v2.py --self-test

LLM test (requires Ollama at 172.20.2.230:11434):
  python3 reasoning_module_v2.py --llm-test
"""
