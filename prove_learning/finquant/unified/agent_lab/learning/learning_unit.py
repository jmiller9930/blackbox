...
def new_learning_unit(
    *,
    ...
    return {
        ...
        # Latest falsification verdicts (chronological); used for validated→active promotion
        "recent_verdict_chain_v1": [],
...
def record_observation(
    ...
    *,
    ...
    # Replay / migration safety for units materialized before newer fields existed.
    unit.setdefault("evidence_record_ids_v1", [])
    unit.setdefault("hit_count_v1", 0)
    unit.setdefault("miss_count_v1", 0)
    unit.setdefault("inconclusive_count_v1", 0)
    ...
    chain = unit.setdefault("recent_verdict_chain_v1", [])
    chain.append(verdict)
    unit["recent_verdict_chain_v1"] = chain[-48:]
...
def update_status(
    ...
    *,
    ...
    unit.setdefault("status_history_v1", []).append({
        "at": now,
        "from": old_status,
        "to": new_status,
        "reason": reason,
